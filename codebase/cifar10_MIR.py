...
from utils.config.configurations import cifar100_large_benign as ds
...
def load_model_metadata():
    ...
    #lr=1e-2
    #wd=1e-3
    #opt = torch.optim.SGD(model.parameters(), lr=lr,momentum=.9, nesterov=True, weight_decay=wd)
...
def retrieve_MIR_samples(Xt, yt,class_imbalance_aware_sample_selection):
    global temp_list,batch_size,total_interfered_samples,VPU_sgd_operations,param_weights_container_Reg_SGD,param_weights_container_vpu_SGD,bool_store_weights,param_diff_list
    ...
    opt = torch.optim.SGD(temp_model.parameters(), lr=learning_rate,momentum=.9, nesterov=True, weight_decay=0.0001)
...
def start_execution():
    global input_shape,tasks,X_test,y_test,test_x,test_y,param_diff_list,error_norm_per_task
    ...
    print("error norm of the tasks",error_norm_per_task)
    # print("param diff is:",param_diff_list)
    # print("otdd list is:",otdd_list)
    if bool_store_weights:
...
if __name__ == "__main__":
    ...
    with open(args.filename, 'w') as fp:
         ...
         test_set_results.extend([e_time,MIR_time,Regular_sgd_operations,VPU_sgd_operations])
