...
if __name__ == "__main__":
    ...
    parser.add_argument('--ds', type=str, default="clear10_papa", metavar='S',help='dataset name')
    ...
    for seed_value in seed_list:
        ...
        proc = subprocess.Popen(["python clear10_PAPA.py --seed="+str(seed_value)+" --gpu="+str(args.gpu)+" --interf_ratio="+str(args.interf_ratio)+" --filename="+str(temp_file_name)],shell=True,cwd=curr_dir)
        proc.communicate()
    ...
    for key, value in auc_results.items():
        # print(key,value)
        pr_auc_o, pr_auc_1,roc_auc,tot_time,mrp_time,r_sgd,v_sgd = value
        print("{:<20}  {:<20}  {:<20}  {:<20} {:<20}  {:<20}  {:<20}  {:<20}".format(key,pr_auc_o, pr_auc_1,roc_auc,tot_time,mrp_time,r_sgd,v_sgd))
    print("-"*80)
    ...
    print("{:<20}  {:<20}  {:<20}  {:<20}  {:<20}  {:<20}  {:<20}  {:<20}".format('avg',float(str(auc_average[0])[:5]), float (str(auc_average[1])[:5]), float(str(auc_average[2])[:5]),float(str(auc_average[3])[:5]),float(str(auc_average[4])[:5]),float(str(auc_average[5])[:5]),float(str(auc_average[6])[:5])))
    print(df.std(axis=1))
    print("-"*80)
