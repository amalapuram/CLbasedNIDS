...
from utils.buffermemory import memory_update,retrieve_replaysamples,memory_update3,memory_update2
...
def ecbrs_train_epoch(Xt, yt,replay_Xt, replay_yt,a):
    ...
    for step, (stream,replay) in enumerate(zip(stream_train_dataloader,replay_train_dataloader)):
        ...
        y_hat = model(x_stream_train.reshape((-1,3,32,32))).reshape(y_stream_train.shape)
...
def train(tasks):
    ...
    for X,y,y_classname in tasks:
        ...
        for curr_batch in range(0, task_size, replay_size):
            ...
            a=1/nc    
            replay_Xt,replay_yt,replay_yname = retrieve_replaysamples(memory_X, memory_y ,memory_y_name,global_priority_list,local_count,replay_size,input_shape,minority_allocation,memory_size,local_store=False,ecbrs_taskaware=ecbrs_taskaware)            
            if ecbrs_taskaware:
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--ds', type=str, default="svhn", metavar='S',help='dataset name')
