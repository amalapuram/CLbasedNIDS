...
from utils.buffermemory import memory_update,retrieve_replaysamples,retrieve_MIR_replaysamples
...
gauss_noise_iter,gauss_noise_threshold = -1,3
gmm,gmm_traintime = None,0
dist_list = list()
interf_ratio = None
...
def fit_gmm():
    ...
    for idx in range(0,len(dist_list)):
        #print("norm is",torch.norm(dist_list[idx],p=1))
        for i in range(0,5):
...
def retrieve_MIR_samples(Xt, yt):
    ...
    stream_train_dataloader = DataLoader(stream_dataset, batch_size=128, shuffle=True) 
    epochs = 5
    ...
    for step,stream in enumerate(stream_train_dataloader):    
        ...
        #print(torch.norm(param_vector,2),torch.norm(param_vector2,2))
        diff = torch.sub(param_vector2,param_vector)    
        #print(torch.norm(param_vector,p=1),torch.norm(param_vector2,p=1),torch.norm( diff,p=1))    
...
def retrieve_MIR_samples_moving_averge():
    global interf_ratio,batch_size,total_interfered_samples,param_weights_queue,param_weights_container_Reg_SGD,param_weights_container_vpu_SGD,bool_store_weights,cos_sim_list,std_noise,gmm
     
    
    
    mean_param_weights = torch.mean(torch.stack(list(param_weights_queue)),0)
    gmm_noise = gmm.sample().to(device)
    mean_param_weights = mean_param_weights + gmm_noise    
    ...
    if MIR_X.shape[0] > 1:
        ...
        if total_weight > 0:
            ...
            rand_indices = np.random.choice(range(0,MIR_X.shape[0]),size=math.ceil(interf_ratio*MIR_X.shape[0]),replace=False,p=weights).tolist()
        else:
            rand_indices = np.random.choice(range(0,len(MIR_X.shape[0])),size=math.ceil(interf_ratio*MIR_X.shape[0]),replace=True).tolist()  
...
def ecbrs_train_epoch(Xt, yt):
    ...
    #model.eval()   
    #y_hat,y_stream_train = None,None    
...
def train_a_task(X,y,y_classname,task_num):
    global memory_X, memory_y, memory_y_name,local_count,global_count,local_store,MIR_time,gmm_traintime
    ...
    task_size = X.shape[0]
    gauss_noise_threshold = min(math.floor((task_size/replay_size)),25)
    ...
    for curr_batch in range(0, task_size, replay_size):
            ...
            gauss_noise_iter = gauss_noise_iter+1
            a=1/nc    
            ...
            beg_time = time.time()
            if gauss_noise_iter < gauss_noise_threshold:
                x_replay_train, y_replay_train,y_name_replay_train,diff = retrieve_MIR_samples(Xt=Xt, yt=yt)
                

            elif gauss_noise_iter == gauss_noise_threshold:
                compute_variance()
                # exit()
                gmm_st = time.time()
                fit_gmm()
                gmm_traintime = time.time()-gmm_st
            else:
            ...
            # update_replay_counter(binarymemorysamples=y_replay_train,classwisememorysamples=y_name_replay_train)
            x_replay_train, y_replay_train,y_name_replay_train = numpyarray_to_tensorarray(x_replay_train),numpyarray_to_tensorarray(y_replay_train),numpyarray_to_tensorarray(y_name_replay_train)
            #Xt = numpyarray_to_tensorarray(Xt).to(device)
            #yt = numpyarray_to_tensorarray(yt).to(device)
...
def taskwise_lazytrain():
    ...
    for task_id,task in enumerate(task_order):
        ...
        input_shape,tasks,X_test,y_test,_,_ = load_dataset(pth,task_class_ids,task_minorityclass_ids,tasks_list,task2_list,[task,],bool_encode_benign=True,bool_encode_anomaly=False,label=label,bool_create_tasks_avalanche=False,load_whole_train_data=False)
...
def start_execution():
    ...
    global input_shape,tasks,X_test,y_test,test_x,test_y,gmm_traintime
    ...
    print("Total execution time is--- %s seconds ---" % (exec_time)  )  
    ...
    print("GMM train time",gmm_traintime)
    if bool_store_weights:
        ...
        np.save(checkpoint_location+"/"+cfg.clstrategy+"_vpu_SGD_",param_weights_container_vpu_SGD) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interf_ratio', type=float, default=0.1, metavar='S',help='interference samples ratio (default: 0.1)')
    s_time = time.time()
    args = parser.parse_args()
    get_gpu(args.gpu)
    auc_result= {}
    interf_ratio = args.interf_ratio
    print("*"*80)
    print("interference ratio:",interf_ratio)
    start_execution()
    e_time = time.time()-s_time
    #evaluate_on_testset()
    with open(args.filename, 'w') as fp:
         test_set_results = evaluate_on_testset()
         test_set_results.extend([e_time,memory_population_time,Regular_sgd_operations,VPU_sgd_operations])
