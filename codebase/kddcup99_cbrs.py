...
from utils.buffermemory import memory_update,retrieve_replaysamples,cbrsmemory_update
...
def train(tasks):
    global memory_X, memory_y, memory_y_name,local_count,global_count,local_store,input_shape,memory_size,task_num,train_size
    ...
    for X,y,y_classname in tasks:
        ...
        task_size = X.shape[0]
        train_size = train_size+task_size
        

def taskwise_lazytrain():
    global test_x,test_y,task_num,train_size
    for task_id,task in enumerate(task_order):
        ...
        input_shape,tasks,X_test,y_test,_,_ = load_dataset(pth,task_class_ids,task_minorityclass_ids,tasks_list,task2_list,[task,],bool_encode_benign=False,bool_encode_anomaly=True,label=label,bool_create_tasks_avalanche=False)
...
def start_execution():
    global input_shape,tasks,X_test,y_test,test_x,test_y,train_size
    ...
    if is_lazy_training:
        ...
        print("train size, test size", train_size,X_test.shape[0])
        exit()
