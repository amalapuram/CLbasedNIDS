...
input_shape,task_order,class_ids,minorityclass_ids,pth,tasks_list,task2_list,label,learning_rate,pth_testset,testset_class_ids = None,None,None,None,None,None,None,None,None,None,None
...
def load_metadata():
    # set_seed(125)
    #get_gpu()
    global task_order,class_ids,minorityclass_ids,pth,tasks_list,task2_list,label,learning_rate,input_shape,pth_testset,testset_class_ids
    global replay_size,memory_size,minority_allocation,epochs,batch_size,device,pattern_per_exp,is_lazy_training, image_resolution
    # set_seed(125)
    #get_gpu()
    label = ds.label
...
def ecbrs_train_epoch(Xt, yt,replay_Xt, replay_yt,a):
    ...
    replay_train_dataloader = DataLoader(replay_dataset, batch_size=128, shuffle=True) 
    ...
    for step, (stream,replay) in enumerate(zip(stream_train_dataloader,replay_train_dataloader)):
      ...
      y_hat_stream = model(x_stream_train.reshape((-1,3,224,224))).reshape(y_stream_train.shape)
      loss_stream = loss_fn(y_hat_stream,y_stream_train)

      y_hat_replay = model(x_replay_train.reshape((-1,3,224,224))).reshape(y_replay_train.shape)
