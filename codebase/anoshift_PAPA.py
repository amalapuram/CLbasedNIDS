...
def compute_variance():
    global dist_list,papa_tie_breaker   
    l1_norm_dist = list()
    for idx in range(len(dist_list)):
        l1_norm_dist.append(torch.norm(torch.abs(dist_list[idx]), p=2))
        
    m = torch.mean(torch.Tensor(l1_norm_dist))
    std = torch.std(torch.Tensor(l1_norm_dist))
    papa_tie_breaker = (std/m).item()
...
def fit_gmm():
    ...
    file_path = 'gmm/'+str(label)
    os.makedirs(file_path, exist_ok = True)
    task_order_index=4
    torch.save(gmm.mean,file_path+'/task_order_'+str(task_order_index)+'_mean.pt')
    torch.save(gmm.variance,file_path+'/task_order_'+str(task_order_index)+'_variance.pt')
    exit()
...
def retrieve_MIR_samples_moving_averge():
    ...
    mean_param_weights = param_weights_queue[-1]+gmm_noise
    torch.nn.utils.vector_to_parameters(mean_param_weights.float(),temp_model.parameters())
    
    # if papa_tie_breaker <0.15:
        
    #     target_param_weights = param_weights_queue[-1] + gmm_noise    
    # else:
    #     target_param_weights = mean_param_weights
        

    # torch.nn.utils.vector_to_parameters(target_param_weights.float(),temp_model.parameters())
    ...
    MIR_samples_indices = MIR_samples_indices.ravel().astype('int').tolist()
    MIR_samples_indices = list(map(bool,MIR_samples_indices))  
    # rand_indices = np.random.choice(range(0,len(MIR_samples_indices)),size=math.ceil(0.1*len(MIR_samples_indices)),replace=True).tolist()  
    ...
    if MIR_X.shape[0] > 1:
        ...
        if total_weight > 0:
            ...
            rand_indices = np.random.choice(range(0,(MIR_X.shape[0])),size=math.ceil(0.15*MIR_X.shape[0]),replace=False).tolist()  

        # rand_indices = np.random.choice(range(0,(MIR_X.shape[0])),size=math.ceil(0.4*MIR_X.shape[0]),replace=False).tolist()  
...
def train_a_task(X,y,y_classname,task_num):
    ...
    task_size = X.shape[0]
    gauss_noise_threshold = min(math.floor((task_size/replay_size))-1,25)
...
def start_execution():
    global input_shape,tasks,X_test,y_test,test_x,test_y,gmm_train_time,VPU_sgd_operations,VPU_sgd_operations
    ...
    print("VirutAL parameters update",VPU_sgd_operations)
