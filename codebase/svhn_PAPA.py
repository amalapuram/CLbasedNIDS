...
def retrieve_MIR_samples_moving_averge():
    ...
    if MIR_X.shape[0] > 1:
        ...
        if total_weight > 0:
            ...
            rand_indices = np.random.choice(range(0,MIR_X.shape[0]),size=math.ceil(0.4*MIR_X.shape[0]),replace=False,p=weights).tolist()
        else:
            rand_indices = np.random.choice(range(0,len(MIR_X.shape[0])),size=math.ceil(0.4*MIR_X.shape[0]),replace=True).tolist()  
