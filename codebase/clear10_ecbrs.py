...
from utils.customdataloader import load_dataset,Tempdataset,compute_total_minority_testsamples,get_inputshape,load_teset,get_balanced_testset
from utils.classifiers import CICIDS2018_FC
from utils.buffermemory import memory_update,retrieve_replaysamples,memory_update2
...
replay_size,memory_size,minority_allocation,epochs,batch_size,device,pattern_per_exp,is_lazy_training,task_num,pth_testset,testset_class_ids = None,None,None,None,None,None,None,None,None,None,None
...
def load_metadata():
    # set_seed(125)
    #get_gpu()
    global task_order,class_ids,minorityclass_ids,pth,tasks_list,task2_list,label,learning_rate,input_shape,ecbrs_taskaware,testset_class_ids,image_resolution 
    global replay_size,memory_size,minority_allocation,epochs,batch_size,device,pattern_per_exp,is_lazy_training,ecbrs_taskaware_memory_X, ecbrs_taskaware_memory_y,ecbrs_taskaware_memory_y_name,memory_per_task,pth_testset
    # set_seed(125)
    #get_gpu()
    label = ds.label
...
def compute_prediction_diff():
    ...
    if image_resolution is not None:
        data = memory_X.reshape(image_resolution)

    data = torch.from_numpy(data).to(device)    
    predictions = model(data).detach().cpu().numpy().squeeze()
    abs_diff = abs(predictions-memory_y)    
...
def ecbrs_train_epoch(Xt, yt,replay_Xt, replay_yt,a):
    ...
    replay_train_dataloader = DataLoader(replay_dataset, batch_size=batch_size, shuffle=True,drop_last=False) 
    ...
    for step, (stream,replay) in enumerate(zip(stream_train_dataloader,replay_train_dataloader)):
        ...
        y_hat = model(x_stream_train.reshape((-1,3,224,224))).reshape(y_stream_train.shape)
...
def train(tasks):
    ...
    for X,y,y_classname in tasks:
        ...
        for curr_batch in range(0, task_size, replay_size):
            ...
            print(replay_individual_count)
            update_exemplars_global_counter(ynamet)        
            total_count=sum(global_count.values())
            a=1/nc    
            replay_Xt,replay_yt,replay_yname = retrieve_replaysamples(memory_X, memory_y ,memory_y_name,global_priority_list,local_count,replay_size,input_shape,minority_allocation,memory_size,ecbrs_taskaware=ecbrs_taskaware,local_store=False)#,pred_diff=compute_prediction_diff().tolist())            
            if ecbrs_taskaware:
            ...
            memory_X, memory_y, memory_y_name,local_count,global_count,local_store,classes_so_far,full,global_priority_list,local_priority_list = memory_update2(Xt,yt,ynamet,task_num,minority_allocation,input_shape,curr_batch,replay_size,memory_size,memory_X, memory_y, memory_y_name,local_count,global_count,local_store,classes_so_far,full,global_priority_list,local_priority_list,model,loss_fn,device,data_reshape=(-1,3,224,224))
...
def start_execution():
    ...
    if is_lazy_training:
        ...
        input_shape,tasks,X_test,y_test,_,_ = load_dataset(pth,class_ids,minorityclass_ids,tasks_list,task2_list,task_order,bool_encode_benign=True,bool_encode_anomaly=False,label=label,bool_create_tasks_avalanche=False)
        initialize_buffermemory(tasks=tasks,mem_size=memory_size)
