...
def load_metadata():
    ...
    minority_allocation = 0.5#ds.minority_allocation
    epochs = ds.n_epochs
...
def train(tasks):
    ...
    for X,y,y_classname in tasks:
        ...
        for curr_batch in range(0, task_size, replay_size):
            ...
            a=1/nc    
            replay_Xt,replay_yt,replay_yname = retrieve_replaysamples(memory_X, memory_y ,memory_y_name,global_priority_list,local_count,replay_size,input_shape,minority_allocation,memory_size,ecbrs_taskaware=ecbrs_taskaware,local_store=True)            
            if ecbrs_taskaware:
