...
def load_metadata(): 
    ...
    task_order = temp_dict['task_order1']
    ...
    minority_allocation = 0.1#ds.minority_allocation
    epochs = ds.n_epochs
...
def retrieve_MIR_samples_moving_averge():
    ...
    MIR_samples_indices = MIR_samples_indices.ravel().astype('int').tolist()
    MIR_samples_indices = list(map(bool,MIR_samples_indices))  
    # rand_indices = np.random.choice(range(0,len(MIR_samples_indices)),size=math.ceil(0.10*len(MIR_samples_indices)),replace=True).tolist()  
    ...
    if MIR_X.shape[0] > 1:
        ...
        if total_weight > 0:
            ...
            rand_indices = np.random.choice(range(0,(MIR_X.shape[0])),size=math.ceil(0.15*MIR_X.shape[0]),replace=True).tolist()  
