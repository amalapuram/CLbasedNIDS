...
from utils.utils import log,create_directories,trigger_logging,set_seed,get_gpu,load_model,compute_otdd_bwtasks
...
def load_metadata():
    ...
    input_shape = 2352#get_inputshape(pth,class_ids)
...
def load_model_metadata():
    ...
    model = load_model(label=label,inputsize=2352)#get_inputshape(pth,class_ids))
...
def retrieve_MIR_samples(Xt, yt,class_imbalance_aware_sample_selection):
    global batch_size,temp_list,total_interfered_samples,VPU_sgd_operations,param_weights_container_Reg_SGD,param_weights_container_vpu_SGD,bool_store_weights
    ...
    for epoch in range(0,epochs):
        for step,stream in enumerate(stream_train_dataloader):
            ...
            y_hat = temp_model(x_stream_train)
            y_hat = y_hat.reshape(y_stream_train.shape)
    ...
    temp_list.append(torch.norm( diff,p=1).item())
...
def taskwise_lazytrain():
    global test_x,test_y,task_num,flops_count,param_diff_list,otdd_list,error_norm_per_task,temp_list
    otdd_taskwise_dist = []
    ...
    #     input_shape,tasks1,X_test,y_test,_,_ = load_dataset(pth,task_class_ids,task_minorityclass_ids,tasks_list,task2_list,[task,],bool_encode_benign=True,bool_encode_anomaly=False,label=label,bool_create_tasks_avalanche=False,load_whole_train_data=load_whole_dataset)
    #     for task_id,task in enumerate(task_order):
    #         task_class_ids = []
    #         task_minorityclass_ids = []
    ...
    #                 task_minorityclass_ids.extend([class_])
    #         print("loading task:",task_id)     
    #         input_shape,tasks2,X_test,y_test,_,_ = load_dataset(pth,task_class_ids,task_minorityclass_ids,tasks_list,task2_list,[task,],bool_encode_benign=True,bool_encode_anomaly=False,label=label,bool_create_tasks_avalanche=False,load_whole_train_data=load_whole_dataset)
    #         curr_dist += compute_otdd_bwtasks(tasks1,tasks2,classlabel=label) 
    #     per_task_avg_disim =  curr_dist/int(no_tasks)   
    #     otdd_taskwise_dist.extend([per_task_avg_disim])
    print("distance is",otdd_taskwise_dist)
    ...
    for task_id,task in enumerate(task_order):
        ...
        train(tasks)
        error_norm_per_task.append(sum(temp_list)/len(temp_list))
        print(error_norm_per_task)
        # vec2 = torch.nn.utils.parameters_to_vector(model.parameters())
        # param_diff_list.append(torch.norm(torch.abs(torch.sub(vec1,vec2)),p=1).detach().cpu().float())
        # if task_id > 0:
        #     # param_diff_list.append(torch.norm(torch.abs(torch.sub(vec1,vec2)),p=1).detach().cpu().float())
        #     otdd_list.append(compute_otdd_bwtasks(previous_task,tasks,classlabel=label))
        # previous_task = tasks
...
def start_execution():
    global input_shape,tasks,X_test,y_test,test_x,test_y,param_diff_list
    ...
    if is_lazy_training:
        test_x,test_y = [],[]
        taskwise_lazytrain()
        print("param",param_diff_list)
        print("otdd",otdd_list)
